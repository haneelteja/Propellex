...
export async function searchProperties(filters: PropertyFilter): Promise<SearchResult> {
  ...
  const dataQuery = `
    SELECT p.id, p.title, p.property_type, p.status, p.price, p.price_per_sqft,
           ...
           p.photos, p.video_url, p.media_sources, p.risk_score, p.roi_estimate_3yr, p.published_at,
...
export async function comparePropertiesWithAI(ids: string[]): Promise<{
  ...
  const rows = await query<{
    ...
    `SELECT id, title, property_type, status, price, price_per_sqft, area_sqft,
            ...
            rera_number, rera_status, photos, video_url, media_sources, risk_score, roi_estimate_3yr,
